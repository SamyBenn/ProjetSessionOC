...
setInterval(function getlireTemp(){
   ...
   setInterval(function getlireTempsCompteur(){
    ...
    xhttp.onreadystatechange = function() {
    if(this.status == 200) {
    document.getElementById("tempsRestantCuisson").innerHTML = this.responseText;
    }
    };
...
function getFromESP_getAllWoodDetail(selectedObject) 
{
    ...
    xhttp.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById("tempsRestantCuisson").innerHTML =responseParsed.drying;
        ...
        document.getElementById("Demarrer").addEventListener("click", demarrerFour);
        envoyerInfo();
    }
